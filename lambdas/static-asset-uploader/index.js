...
function cleanS3Bucket (bucketName) {
  ...
  return exports.s3.listObjectsV2(params).promise()
    .then(result => {
      ...
      const keys = _.map(result.Contents, (obj) => {
...
function createCatalogDirectory (staticBucketName) {
  const params = { Bucket: staticBucketName, Key: 'catalog/', Body: '' }
...
function addConfigFile (bucketName, event) {
  ...
  const suffix = event.ResourceProperties.MarketplaceSuffix
...
function processFile (fileStat, readPromises, uploadPromises, bucketName, event, context) {
  ...
  const thisReadPromise = fse.readFile(filePath)
...
function uploadStaticAssets (bucketName, event, context) {
  const excludeDirFilter = excludeDirFactory()
  const excludeCustomContentFilter = excludeCustomContentFactory(event.RequestType, event.ResourceProperties.RebuildMode)
  const readPromises = []
